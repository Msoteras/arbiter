...
class CaseControllerTest {
    ...
    void listCases_noFilters_returnsPagedContent() throws Exception {
        ...
        when(caseService.listCases(isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), eq(false), eq(false), eq(false), eq(false), eq(false), eq(false), isNull(), eq(CaseScope.ALL), isNull(), eq(DEFAULT_PAGEABLE)))
    ...
    void listCases_sortByResponseDeadline_isAccepted() throws Exception {
        ...
        when(caseService.listCases(isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(),
                isNull(), isNull(), eq(false), eq(false), eq(false), eq(false), eq(false), eq(false), isNull(), eq(CaseScope.ALL), isNull(), eq(byDeadline)))
    ...
    void listCases_withStatusFilter_passesStatusThrough() throws Exception {
        ...
        when(caseService.listCases(eq(List.of(CaseStatus.PENDING_ANALYST_REVIEW)), isNull(), isNull(), isNull(),
                isNull(), isNull(), isNull(), isNull(), isNull(), eq(false), eq(false), eq(false), eq(false), eq(false), eq(false), isNull(), eq(CaseScope.ALL), isNull(), eq(DEFAULT_PAGEABLE)))
    ...
    void listCases_withInsuredIdFilter_passesInsuredIdThrough() throws Exception {
        CaseResponse response = caseResponse(1L, CaseStatus.PENDING_CLASSIFICATION);
        when(caseService.listCases(isNull(), isNull(), isNull(), eq("40.123.456"), isNull(), isNull(), isNull(), isNull(), isNull(), eq(false), eq(false), eq(false), eq(false), eq(false), eq(false), isNull(), eq(CaseScope.ALL), isNull(), eq(DEFAULT_PAGEABLE)))
                .thenReturn(new PageImpl<>(List.of(response), DEFAULT_PAGEABLE, 1));
    ...
    void listCases_withRiskBandFilter_passesRiskBandThrough() throws Exception {
        CaseResponse response = caseResponse(1L, CaseStatus.PENDING_ANALYST_REVIEW);
        when(caseService.listCases(isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(),
                eq(RiskBand.HIGH), isNull(), eq(false), eq(false), eq(false), eq(false), eq(false), eq(false), isNull(), eq(CaseScope.ALL), isNull(), eq(DEFAULT_PAGEABLE)))
                .thenReturn(new PageImpl<>(List.of(response), DEFAULT_PAGEABLE, 1));
    ...
    void listCases_withAssignedToMe_passesTheLensThrough() throws Exception {
        ...
        CaseResponse response = caseResponse(1L, CaseStatus.PENDING_ANALYST_REVIEW);
        when(caseService.listCases(isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(),
                isNull(), isNull(), eq(true), eq(false), eq(false), eq(false), eq(false), eq(false), isNull(), eq(CaseScope.ALL), isNull(), eq(DEFAULT_PAGEABLE)))
                .thenReturn(new PageImpl<>(List.of(response), DEFAULT_PAGEABLE, 1));
    ...
    @Test
    void listCases_withReportReceived_passesTheFilterThrough() throws Exception {
        CaseResponse response = caseResponse(1L, CaseStatus.PENDING_ANALYST_REVIEW);
        when(caseService.listCases(isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(),
                isNull(), isNull(), eq(false), eq(false), eq(false), eq(false), eq(false), eq(true), isNull(), eq(CaseScope.ALL), isNull(), eq(DEFAULT_PAGEABLE)))
                .thenReturn(new PageImpl<>(List.of(response), DEFAULT_PAGEABLE, 1));

        mockMvc.perform(get("/api/v1/cases").param("reportReceived", "true"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content.length()").value(1));
    }
    ...
    void listCases_withFreeTextSearch_passesQThrough() throws Exception {
        CaseResponse response = caseResponse(1L, CaseStatus.PENDING_ANALYST_REVIEW);
        when(caseService.listCases(isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), eq("POL-CEL"), isNull(), isNull(), eq(false), eq(false), eq(false), eq(false), eq(false), eq(false), isNull(), eq(CaseScope.ALL), isNull(), eq(DEFAULT_PAGEABLE)))
                .thenReturn(new PageImpl<>(List.of(response), DEFAULT_PAGEABLE, 1));
    ...
    void listCases_withClaimCausePolicyNumberAndDateRange_passesFiltersThrough() throws Exception {
        ...
        when(caseService.listCases(isNull(), eq("Robo en vía pública"), eq("POL-CEL-2024-001"), isNull(),
                eq(LocalDate.of(2026, 6, 1)), eq(LocalDate.of(2026, 6, 30)), isNull(), isNull(), isNull(), eq(false), eq(false), eq(false), eq(false), eq(false), eq(false), isNull(), eq(CaseScope.ALL), isNull(), eq(DEFAULT_PAGEABLE)))
                .thenReturn(new PageImpl<>(List.of(response), DEFAULT_PAGEABLE, 1));
    ...
    void listCases_withPageParams_passesPageableThrough() throws Exception {
        ...
        when(caseService.listCases(isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), eq(false), eq(false), eq(false), eq(false), eq(false), eq(false), isNull(), eq(CaseScope.ALL), isNull(), eq(pageable)))
