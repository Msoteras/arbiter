...
8. **Auth0 + JWT + RBAC — estado objetivo.** Tres roles: `ASEGURADO`, `ANALISTA_SINIESTROS`, `REFERENTE_ASEGURADORA`.
   - **Paso transitorio (H0001, actual):** `auth-service` implementa login propio contra una tabla `users` (BCrypt + JWT firmado por nosotros + bloqueo de cuenta tras 5 intentos fallidos por 15 min), porque Auth0 tiene costo a escala y no queríamos bloquear el resto del flujo por eso. **No es la decisión final** — es la opción 1 evaluada explícitamente contra "arrancar directo con Auth0" y "Auth0Adapter con mock", elegida para poder seguir con RBAC end-to-end sin depender de configurar un tenant de Auth0 primero.
   - **Migración futura:** la validación de credenciales está aislada detrás de la interfaz `CredentialsAuthenticator` (`auth-service/services`) — el swap a Auth0 es una nueva implementación (`Auth0CredentialsAuthenticator` o similar) que la reemplaza. `AuthController`, `AuthService`, `JwtService` y el contrato de `POST /auth/login` (request/response) no deberían cambiar. Cuando se migre, retirar `DatabaseCredentialsAuthenticator`, la tabla `users` y el bloqueo por intentos (Auth0 ya trae su propia protección de fuerza bruta).
   - **H0002 (Alta de Usuarios) — misma lógica de "paso transitorio":** la Historia de Usuario pide que el sistema mande un mail con credenciales provisorias (necesita SendGrid); Trello pide que el referente tipee la contraseña directo en el form. Elegimos **contraseña directa, sin SendGrid** — mismo criterio que con Auth0: primera versión simple, sin sumar una integración externa nueva todavía. Si el equipo lo pide, se puede migrar al flujo de mail + link de reseteo después sin tocar el contrato del endpoint (agregar un `passwordHash` temporal + un token de reseteo, no cambiar `POST /api/v1/auth/users`).
   - **H0002 — alcance de roles:** el alta de usuarios en este pase **solo crea cuentas `ANALISTA_SINIESTROS`** (es lo único que detalla el criterio concreto de la historia: campos Sector + Fecha de Ingreso solo tienen sentido para un empleado interno). El asegurado **no se da de alta por este panel** — ya existe como cliente con póliza en la BD de la aseguradora (ver punto 10), y H0004 asume que el "asegurado registrado" ya existe antes de la denuncia. Dejamos preparado el modelo (`User.insuredId`, nullable) para cuando se decida cómo el referente vincula/habilita el acceso de un asegurado existente — no es una migración nueva, es agregar `ASEGURADO` a los roles permitidos + validar el DNI.
   - **H0002 — pendiente, no implementado:** la pantalla de Trello para listar/editar el rol de usuarios ya creados (`GET/PUT /auth/usuarios`) es de la card "Gestión de roles y permisos", separada de "Alta de Usuarios". No entra en este pase — solo alta. Retomar cuando se aborde esa card explícitamente.
...
- Spring Data JPA + JDBC (PostgreSQL driver). **En la práctica no usamos Flyway/Liquibase**: cada módulo corre con `hibernate.ddl-auto: update` y el schema + datos de prueba se mantienen a mano en `db/init.sql` (todas las tablas de todos los módulos, un solo script). Seguir ese patrón para módulos nuevos salvo que el equipo decida adoptar una herramienta de migraciones explícitamente.
