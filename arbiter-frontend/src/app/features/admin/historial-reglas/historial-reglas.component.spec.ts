...
describe('HistorialReglasComponent', () => {
  ...
  it('shows the field with its Spanish label, not the JSON key', async () => {
    await mount([entry()]);
  ...
  it('without a label falls back to the last path segment and splits the qualifier', async () => {
    await mount([
      entry({
        ruleType: 'SCORING',
  ...
  it('shows the risk band by name, not by enum literal', async () => {
    await mount([
      entry({
        ruleType: 'SCORING',
        changes: [
  ...
  it('names the resolution target and its fields', async () => {
    await mount([
      entry({
  ...
  it('translates the overdue-payment action instead of showing the literal', async () => {
    await mount([
      entry({
  ...
  it('does not repeat the factor key as another field', async () => {
    await mount([
      entry({
        ruleType: 'SCORING',
        changes: [
  ...
  it('shows only what was removed and added in a list field', async () => {
    await mount([
      entry({
        ruleType: 'FAST_TRACK',
        changes: [
  ...
  it('shows the required documents by name, not by code', async () => {
    await mount([
      entry({
        ruleType: 'FAST_TRACK',
        changes: [
  ...
  it('shows the author the backend resolves, not the whole reason', async () => {
  ...
  it('leaves no dangling "por" when the reason names nobody', async () => {
  ...
  it('shows the rule creation with no previous version', async () => {
  ...
  it('says "Se agregó" when the field did not exist before', async () => {
    await mount([
      entry({
        ruleType: 'SCORING',
  ...
  it('shows the scope of an insurer-wide rule instead of leaving it empty', async () => {
    await mount([
