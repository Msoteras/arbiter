...
### Un desvío del entorno (resuelto)

> Ya no aplica: `InsurerDatabaseAdapter` resuelve la BD Aseguradora por esquema de tenant y no usa
> `aseguradora_id`. Queda como registro de por qué las primeras corridas usaron el mock.

**`InsurerDatabaseAdapter` no hablaba con la BD real.** El código —en `classification-service`, en
