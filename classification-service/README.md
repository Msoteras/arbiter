...
│   ├── adapters/
│   │   ├── ClaimClassifier.java      # classifier interface
...
│   ├── dto/
│   │   ├── ClassificationRequest.java     # classifier input
│   │   └── ClassificationResponse.java    # output (classification + factors + confidence)
│   └── exceptions/
│       └── InvalidClassificationException.java
├── src/main/resources/
│   ├── application.yml
│   └── prompts/
...
└── src/test/
    ...
    │   └── MockClaimClassifier.java             # mock for unit tests
    └── resources/fixtures/
...
Starts Ollama + classification-service in containers:
...
When you see `Started ClassificationServiceApplication` → ready at `http://localhost:8082`
...
```json
{
  ...
  "expectedClassification": "FALTA_DOCUMENTACION | LLM_RECOMIENDA_APROBAR | LLM_NO_RECOMIENDA_APROBAR | LLM_SOLICITA_REVISION_MANUAL",
  "request": {
    "branch": "...",
    "product": "...",
    "claimCause": "...",
...
To add a new scenario: create a JSON with the same structure in `fixtures/` and add it to the `@ValueSource` in `OllamaClassificationScenariosTest`.
...
3. Select: `Arbiter_Claims_Classification.postman_collection.json`
...
Claim (frontend/Postman)
    │
    ▼
ClassificationController ──► ClassificationOrchestrator
                                    │
                    ┌───────────────┼───────────────┐
                    ▼               ▼               ▼
            InsurerAdapter  RulesAdapter   ClaimClassifier
            (policy+history)    (branch rules)  (Ollama/LLM)
                    │               │               │
                    ▼               ▼               ▼
              Mock / REST      Mock / REST     OllamaAdapter
              (insurer DB)     (rules-service) (qwen3-vl)
```

In dev/test the mocks are used (`MockInsurerAdapter`, `MockRulesAdapter`).
