...
@SpringBootTest
class OllamaClassificationScenariosTest extends AbstractPersistenceIT {
    ...
    @Autowired
    private ClaimClassifier classifier;
    ...
    // Note: FAST_TRACK is no longer a possible LLM output — it's decided beforehand,
    // deterministically, by FastTrackValidator (see ClassificationOrchestrator).
    // Fast Track scenarios were left out of this test because they no longer apply at this level.
    ...
    void classifyScenario(String scenario) throws IOException {
        ...
        Classification expected = Classification.valueOf(fixture.get("expectedClassification").asText());
