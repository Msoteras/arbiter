...
BEGIN
    ...
    -- One row per case — the analyst derives once.
    EXECUTE format($ddl$
        CREATE TABLE %I.expert_assessment (
            ...
            -- A verdict with no date, or a date with no verdict, is half a return. The
            -- report lands whole or it did not land.
    ...
    -- Política de derivación a peritaje, una por ramo (coverage_id NULL: que un peritaje se
    -- pague solo depende del tipo de bien, no de qué cobertura se afectó). El monto mínimo es
    -- lo que la hace configurable: el peritaje tiene un costo fijo, así que abajo de cierto
    -- reclamo sale más caro que el siniestro. Una aseguradora que solo vende celulares de
    -- ticket bajo simplemente no carga esta regla y sus analistas nunca ven la opción de
    -- derivar — es opt-in, y por eso `enabled=false` es la respuesta cuando no hay fila.
    -- effect DERIVAR ya estaba previsto en el DER (ver InsurerRule.effect).
    EXECUTE format($ddl$
        INSERT INTO %I.insurer_rule (id, active, valid_from, name, rule_type, effect, priority,
                                     blocks_fast_track, branch_id, coverage_id, configuration) VALUES
